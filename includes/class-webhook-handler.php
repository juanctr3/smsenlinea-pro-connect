...
class Webhook_Handler {
    ...
    public function process_webhook( \WP_REST_Request $request ) {
        ...
        // 1. Validación de Seguridad [cite: 83]
        if ( ! isset( $params['secret'] ) || $params['secret'] !== $this->webhook_secret ) {
            return new \WP_Error( 'forbidden', 'Invalid Secret', [ 'status' => 403 ] );
        }

        // 2. Extraer datos según tipo [cite: 30]
        $type = $params['type'] ?? '';
        $data = $params['data'] ?? [];

        // Solo nos interesan mensajes entrantes (SMS o WhatsApp) [cite: 6]
        if ( ! in_array( $type, [ 'sms', 'whatsapp' ] ) ) {
            return rest_ensure_response( [ 'success' => true, 'msg' => 'Ignored type' ] );
        }

        // Normalizar datos (WhatsApp usa 'wid' o 'phone', SMS usa 'phone') [cite: 40, 49]
        $incoming_phone = isset( $data['wid'] ) ? $data['wid'] : ( $data['phone'] ?? '' );
        $incoming_msg   = trim( $data['message'] ?? '' ); // [cite: 41, 51]
    ...
    private function handle_conversational_logic( $phone, $message ) {
        ...
        if ( $message_upper === 'SI' ) {
            ...
            $support_link = home_url( '/contacto' ); // O link directo a WhatsApp de soporte humano
        ...
        elseif ( $message_upper === 'NO' ) {
            ...
            // Simplemente borramos el transient para no molestar más
            delete_transient( 'smsenlinea_recovery_' . $clean_phone );
            
            // Opcional: Confirmación de salida
            //$api->send_notification( $phone, "Entendido, no te enviaremos más mensajes sobre este pedido.", 'whatsapp' );
        }
    }
}